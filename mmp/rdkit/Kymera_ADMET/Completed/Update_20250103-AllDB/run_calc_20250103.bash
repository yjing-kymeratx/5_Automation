...
JobName="Update_$dateToday-AllDB"    # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
inFileName="$RootDir/DataView_All_MMPs_AllProject_1__export.csv"    # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
## ------------------ Check MMPDB info ------------------
cd $JobDirNew
$bash2py mmpdb list --all ./results/Compounds_All.mmpdb 2>&1 | tee ./results/MMPDB_info.txt