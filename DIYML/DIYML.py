...
def __cols_x_selection(ML_dataSet, desc_cx, desc_cx_norm, desc_rd, desc_rd_norm):
    print("desc_cx", desc_cx, "desc_cx_norm", desc_cx_norm, "desc_rd", desc_rd, "desc_rd_norm", desc_rd_norm)
    cols_x_sele_prefix = []
    if desc_cx and desc_cx_norm:
        cols_x_sele_prefix.append('cx_')
    if desc_rd and desc_rd_norm:
        cols_x_sele_prefix.append('rd_')
    print(f"\tColName_sele prefix: {cols_x_sele_prefix}")
    
    print(f"Original dataTable has {len(ML_dataSet.X_Training.columns)} columns")
    cols_x_sele = [col for col in ML_dataSet.X_Training.columns if col[0:3] in cols_x_sele_prefix]
    return cols_x_sele
...
def step_0_Args_Prepation(parser_desc):
    ...
    parser.add_argument('--desc_cx_norm', action="store", default='False', help='<True> for normalizaing ChemAxon Molecular Property')
    parser.add_argument('--desc_rd_norm', action="store", default='False', help='<True> for normalizaing RDKit Molecular Property')
...
def step_1_Data_Preparation(args, dataName="my_ML_dataset", saveDict=True, tmpFolder='./tmp'):
    ...
    my_ML_Data = ML_Dataset.Data4ML(dataName=dataName)
    ...
    my_ML_Data.prepare_dataset(dsLabels=['Training', 'Validation', 'Test'], istrain=True)
...
###################################### prepare dataset #######################################
##############################################################################################
def step_2_Data_Preprocessing(ML_dataSet, args, saveProcessor=True, tmpFolder='./tmp'):
    transformType = args.yTransform
    desc_cx = _str_2_bool(args.desc_cx)
    desc_rd = _str_2_bool(args.desc_rdkit)
    desc_cx_norm = _str_2_bool(args.desc_cx_norm)
    desc_rd_norm = _str_2_bool(args.desc_rd_norm)
    cols_x_sele =__cols_x_selection(ML_dataSet, desc_cx, desc_cx_norm, desc_rd, desc_rd_norm)
    ...
    ML_dataSet.setAttributes(f"_dict_dataSet", dataDict_ds)
    ...
    return dataDict_ds, my_preProcessor
...
####################################### output preds #########################################
##############################################################################################
def step_4_save_results(ML_dataSet, dataDict_model):
    dataTable_raw = ML_dataSet._dataTable
    dataDict_perf = {}
    curFolder = os.getcwd()
    os.makedirs(f'{curFolder}/models') if not os.path.exists(f'{curFolder}/models') else print(f'\t---->{curFolder}/models/ is existing\n')

    
    for ml_method in dataDict_model:
        ml_predictions = dataDict_model[ml_method].predictions
        ml_pred_merged = pd.merge(dataTable_raw, ml_predictions, left_index=True, right_index=True)

        ## ------------- save predictions details for each method -------------
        os.makedirs(f'{curFolder}/models/{ml_method}') if not os.path.exists(f'{curFolder}/models/{ml_method}') else print(f'\t---->{curFolder}/models/{ml_method} is existing\n')
        os.makedirs(f'{curFolder}/models/{ml_method}/pred_results') if not os.path.exists(f'{curFolder}/models/{ml_method}/pred_results') else print(f'\t---->{curFolder}/models/{ml_method}/pred_results is existing\n')

        csv_tmp = f'{curFolder}/models/{ml_method}/pred_results/results_{ml_method}.csv'
        ml_pred_merged.to_csv(csv_tmp)
        print(f"Results of {ml_method} model has been saved to {csv_tmp}")

        ## ------------- performance -------------
        dataDict_perf[ml_method] = {}
        dataDict_perf[ml_method]['ML_method'] = ml_method
        ml_performance = dataDict_model[ml_method].performance
        for label_ds in ml_performance:
            for perf_type in ml_performance[label_ds]:
                ds_perf = label_ds + '_' + perf_type
                dataDict_perf[ml_method][ds_perf] = ml_performance[label_ds][perf_type]
    dataTable_perf = pd.DataFrame.from_dict(dataDict_perf).T
    os.makedirs(f'{curFolder}/pred_results') if not os.path.exists(f'{curFolder}/pred_results') else print(f'\t---->{curFolder}/pred_results/ is existing\n')
    dataTable_perf.to_csv(f'{curFolder}/pred_results/All_performance.csv', index=False)

##############################################################################################
####################################### output all packages ##################################
##############################################################################################
def step_5_export_model_package(ML_dataSet, preProcessor, dict_models):
    curFolder = os.getcwd()    # '.'
    os.makedirs(f'{curFolder}/models') if not os.path.exists(f'{curFolder}/models') else print(f'\t---->{curFolder}/models/ is existing\n')
    
    for ml_method in dict_models:
        model_package = {}
        model_package['dataset'] = ML_dataSet
        model_package['preProcessor'] = preProcessor
        model_package['DIYML_model'] = dict_models[ml_method]
        
        os.makedirs(f'{curFolder}/models/{ml_method}') if not os.path.exists(f'{curFolder}/models/{ml_method}') else print(f'\t---->{curFolder}/models/{ml_method} is existing\n')
        fileName_modelPkg = f'{curFolder}/models/{ml_method}/{ml_method}.model'
        with open(fileName_modelPkg, 'wb') as mofh:
            pickle.dump(model_package, mofh)
            print(f'\tThe model package <{ml_method}.model> has been saved to {fileName_modelPkg}\n')   
    return None
...
def main():
    ...
    print(f"\n==>Step 2 Normalize the descriptor and take log on y ...")
    dataDict_ds, my_preProcessor = step_2_Data_Preprocessing(ML_dataSet, args, saveProcessor=True, tmpFolder=tmpFolder)
    ...
    ## -------------- step 4 save results --------------
    step_4_save_results(ML_dataSet, dataDict_model)

    ## -------------- step 5 export model package for making prediction --------------
    step_5_export_model_package(ML_dataSet, my_preProcessor, dataDict_model)
